...
export default function ContextPage() {
  ...
  const [matchesDevice, setMatchesDevice] = useState(null); // true/false
    ...
    try {
      const payload = {
        ...
        context: {
          ...
          required_throughput: form.required_throughput,
          min_throughput_mbps: form.required_throughput ? parseFloat(form.min_throughput) : null,
      ...
      setResult(data); // expects { cipher, explanation } shape from backend
    } catch (err) {
...
<div className="relative px-12 pb-10 flex justify-center">
  <div className="max-w-6xl w-full flex gap-6 items-start">
    ...
    <div className="flex-[1.6] min-w-[500px] flex flex-col">
      ...
      {/* Context form card — fully independent height */}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => update("required_throughput", true)}
                  className={`flex-1 py-2.5 rounded-md text-sm font-semibold transition ${
                    form.required_throughput === true
                      ? "bg-gradient-to-b from-[#7aa3ff] to-[#4a7bef] text-[#0a0a0a]"
                      : "border border-white/15 text-[#c7c7c7] hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => update("required_throughput", false)}
                  className={`flex-1 py-2.5 rounded-md text-sm font-semibold transition ${
                    form.required_throughput === false
                      ? "bg-gradient-to-b from-[#7aa3ff] to-[#4a7bef] text-[#0a0a0a]"
                      : "border border-white/15 text-[#c7c7c7] hover:bg-white/5"
                  }`}
                >
            ...
            <div className="min-w-0">
              ...
              <div className={`flex gap-1.5 ${form.required_throughput !== true ? "opacity-40" : ""}`}>
                <input
                  type="number"
                  value={form.min_throughput}
                  onChange={(e) => update("min_throughput", e.target.value)}
                  disabled={form.required_throughput !== true}
                  className="w-0 flex-1 min-w-0 bg-[#0a0a0a] border border-white/15 rounded-md px-3 py-2.5 text-sm focus:outline-none focus:border-[#5b8cff] disabled:cursor-not-allowed"
                />
                <span className="flex-none flex items-center px-2.5 text-xs text-[#8a8a8a] border border-white/15 rounded-md">
                  Mbps
                </span>
              </div>
            </div>
    ...
    <div className="flex-1 min-w-[320px] flex flex-col">
      {/* Invisible spacer — mirrors the left title block's height exactly, so the result card lines up with Context Specification without coupling heights */}
      ...
      {/* Result card — fully independent height */}
      <div className="bg-[#121212] border border-white/10 rounded-2xl p-4 shadow-2xl">
        <div className="flex items-center gap-2 mb-4">
