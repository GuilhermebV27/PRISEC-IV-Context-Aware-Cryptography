 ...
 * ChaCha20
 * SPECK
 * RECTANGLE (AVX2 bit-sliced implementation, 256 blocks/batch; absorbed
 *            from the former benchmark7.c - requires runtime AVX2 support,
 *            unlike the rest of this file, which runs on any 64-bit CPU.
 *            The portable, non-AVX2 bitslice version now lives in
 *            benchmark5.c as the software-only comparison point.)
 * HIGHT
 *
 * Build:
 *   gcc -O2 -mavx2 -fno-stack-protector -o benchmark1 benchmark1.c -lcrypto -lm \
...
static inline int get_inner_loops(size_t size_n)
{
    if (size_n <= 1 * 1024)
        return 10000;
    else if (size_n <= 5 * 1024)
        return 5000;
    else if (size_n <= 10 * 1024)
        return 2000;
    else if (size_n <= 50 * 1024)
        return 200;
    else if (size_n <= 100 * 1024)
        return 100;
    else if (size_n <= 1 * 1024 * 1024)
        return 50;
    else if (size_n <= 5 * 1024 * 1024)
        return 10;
    else if (size_n <= 10 * 1024 * 1024)
        return 5;
    else
        return 1;
}
                        ...
                        uint8_t **out, size_t *out_len)
{
    ...
    if (!buf)
        return 0;
    if (!aes_encrypt(key, key_len, in, in_len, buf, out_len))
    {
        free(buf);
        return 0;
    }
                        ...
                        uint8_t **out, size_t *out_len)
{
    ...
    if (!buf)
        return 0;
    if (!aes_decrypt(key, key_len, in, in_len, buf, out_len))
    {
        free(buf);
        return 0;
    }
                           ...
                           uint8_t **out, size_t *out_len)
{
    ...
    if (!buf)
        return 0;
    if (!chacha20_encrypt(key, key_len, in, in_len, buf, out_len))
    {
        free(buf);
        return 0;
    }
                           ...
                           uint8_t **out, size_t *out_len)
{
    ...
    if (!buf)
        return 0;
    if (!chacha20_decrypt(key, key_len, in, in_len, buf, out_len))
    {
        free(buf);
        return 0;
    }
...
/* RECTANGLE now uses the AVX2 bit-sliced implementation (256 blocks/batch,
 * requires runtime AVX2 support), absorbed from the former benchmark7.c,
 * since it consistently outperforms both the scalar and portable-bitslice
 * paths. The portable bitslice implementation is preserved in benchmark5.c
 * as the software-only (no-AVX2) comparison point. */
...
static algo_t ALGOS[] = {
    ...
    {"RECTANGLE", RECT_KEY_SIZE, 1, RECT_BLOCK_SIZE, wrap_rect_enc, wrap_rect_dec},
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr,
                "This CPU (or this build) does not support AVX2; benchmark1's "
                "RECTANGLE entry now requires it (absorbed from benchmark7.c). "
                "Build/run on an AVX2-capable machine, or fall back to the "
                "portable bitslice RECTANGLE entry in benchmark5.c.\n");
        return 1;
    }
