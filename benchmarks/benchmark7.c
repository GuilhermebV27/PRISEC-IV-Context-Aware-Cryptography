/*
* benchmark7.c - PRISEC-IV Phase 7 Benchmark (RECTANGLE: scalar vs bitslice)
*
* Head-to-head comparison of the two RECTANGLE implementations already
* present in rectangle.h, using the same single-cipher harness as
* benchmark1.c (no ECC, no cascade):
*
*   RECTANGLE      -> rectangle_encrypt/rectangle_decrypt (scalar,
*                      table-driven S-box, one block at a time)
*   RECTANGLE-BS   -> rectangle_bitslice_encrypt/rectangle_bitslice_decrypt
*                      (bitsliced, S-box as boolean logic over 64-block
*                      batches packed into plain uint64_t "planes" —
*                      no AVX/SIMD intrinsics involved, just wider,
*                      denser bit-level parallelism on ordinary registers)
*
* Since the bitslice path transposes data into 64-block batches internally,
* it is expected to lose to the scalar path on tiny inputs (transpose
* overhead dominates) and win on larger inputs where full batches amortize
* that cost — this benchmark exists to actually measure that crossover
* point rather than assume it.
*
* Build:
*   gcc -O2 -fno-stack-protector -o benchmark7 benchmark7.c -lcrypto -lm \
...
static int wrap_rect_bs_enc(const uint8_t *key, int key_len,
                             const uint8_t *in, size_t in_len,
                             uint8_t **out, size_t *out_len) {
    (void)key_len;
...
static int wrap_rect_bs_dec(const uint8_t *key, int key_len,
                             const uint8_t *in, size_t in_len,
                             uint8_t **out, size_t *out_len) {
    (void)key_len;
...
static algo_t ALGOS[] = {
    ...
    { "RECTANGLE-BS", RECT_KEY_SIZE, 1, RECT_BLOCK_SIZE, wrap_rect_bs_enc, wrap_rect_bs_dec },
};
