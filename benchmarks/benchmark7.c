/*
* benchmark7.c - PRISEC-IV Phase 7 Benchmark (RECTANGLE-AVX2)
*
* Tests the AVX2 bit-sliced RECTANGLE implementation (rectangle_avx2.h,
* 256 blocks/batch, requires runtime AVX2 support) everywhere RECTANGLE
* appears outside of an ECC context:
*
*   RECTANGLE-AVX2          (single cipher, benchmark1-style, no ECC)
*   HIGHT+RECTANGLE-AVX2    (cascade, benchmark3-style, no ECC —
*                            the only non-ECC cascade that includes
*                            RECTANGLE; HIGHT stays scalar)
*
* This benchmark requires AVX2 and exits immediately if the CPU/build
* doesn't support it (unlike phase1/phase3, which now use the portable
* uint64_t bit-slice and run on any 64-bit CPU with no such requirement).
*
* Build:
* gcc -O2 -mavx2 -fno-stack-protector -o benchmark7 benchmark7.c -lcrypto -lm \
...
static int wrap_rect_avx2_enc(const uint8_t *key, int key_len,
const uint8_t *in, size_t in_len,
uint8_t **out, size_t *out_len) {
(void)key_len;
uint8_t *buf = rectangle_avx2_encrypt(key, in, in_len, out_len);
if (!buf) return 0;
*out = buf;
return 1;
}
static int wrap_rect_avx2_dec(const uint8_t *key, int key_len,
const uint8_t *in, size_t in_len,
uint8_t **out, size_t *out_len) {
(void)key_len;
uint8_t *buf = rectangle_avx2_decrypt(key, in, in_len, out_len);
if (!buf) return 0;
*out = buf;
return 1;
}
...
static algo_t RECTANGLE_AVX2 = { "RECTANGLE-AVX2", RECT_KEY_SIZE, 1, RECT_BLOCK_SIZE,
                                  wrap_rect_avx2_enc, wrap_rect_avx2_dec };
static algo_t HIGHT_ = { "HIGHT", HIGHT_KEY_SIZE, 1, HIGHT_BLOCK_SIZE,
                          wrap_hight_enc, wrap_hight_dec };
...
int main(int argc, char **argv) {
    if (!rectangle_avx2_available()) {
        fprintf(stderr,
            "This CPU (or this build) does not support AVX2; benchmark7 "
            "measures RECTANGLE-AVX2 specifically and cannot fall back to "
            "a scalar path. Use phase1/phase3 (portable bit-slice) instead.\n");
        return 1;
    }
    ...
    FILE *csv = fopen("phase7_results.csv", "w");
    if (!csv) {
        fprintf(stderr, "Failed to open phase7_results.csv for writing\n");
        return 1;
    }
    ...
    /* Single cipher: RECTANGLE-AVX2 alone. */
    for (int s = 0; s < N_SIZES; s++) {
        int pipefd[2];
        if (pipe(pipefd) != 0) {
            fprintf(stderr, "pipe() failed for %s %s\n", RECTANGLE_AVX2.name, SIZES[s].label);
            continue;
        }
        pid_t pid = fork();
        if (pid < 0) {
        ...
        if (pid == 0) {
            ...
            result_t res = run_combo_single(&RECTANGLE_AVX2, &SIZES[s]);
        ...
        write_row(csv, RECTANGLE_AVX2.name, SIZES[s].label, &res, RECTANGLE_AVX2.is_block_cipher);
        fflush(csv);

        printf("[parent] [%s | %s] finished -> enc_ms=%.4f dec_ms=%.4f mem_enc_peak_kb=%.4f mem_dec_peak_kb=%.4f\n\n",
               RECTANGLE_AVX2.name, SIZES[s].label, res.enc_ms, res.dec_ms,
               res.mem_enc_peak_kb, res.mem_dec_peak_kb);
        fflush(stdout);
    }

    /* Cascade: HIGHT+RECTANGLE-AVX2. */
    ...
    fclose(csv);
    printf("Done. Results written to phase7_results.csv\n");
    return 0;
}
