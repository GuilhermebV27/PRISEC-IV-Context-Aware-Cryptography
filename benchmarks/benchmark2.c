/*
* benchmark2.c - PRISEC-IV Phase 2 Benchmark (cascade / dual-layer encryption)
*
* [Formerly benchmark3.c / Phase 3. Renumbered to Phase 2 after benchmark2.c
* (ECC + single cipher) and benchmark4.c (ECC + cascade) were removed.]
*
* Cascade pairs (layer1+layer2, named Stronger+Weaker, matching
* benchmark5/6/8.c's convention):
*   AES-256+AES-128
*   AES-128+HIGHT
*   AES-128+SPECK
*   AES-256+ChaCha20
*   ChaCha20+SPECK
*   SPECK+HIGHT
*   RECTANGLE+HIGHT (RECTANGLE now runs the AVX2 bit-sliced implementation,
*     256 blocks/batch, requires runtime AVX2 support; absorbed from the
*     former benchmark7.c, which mirrored this same cascade as
*     "HIGHT+RECTANGLE-AVX2". HIGHT stays scalar. The portable, non-AVX2
*     bitslice single-cipher RECTANGLE now lives in benchmark5.c as the
*     software-only comparison point.)
*
* Build:
*   gcc -O2 -mavx2 -fno-stack-protector -o benchmark2 benchmark2.c -lcrypto -lm \
 ...
 * HIGHT stays on its scalar implementation. */
...
static algo_t RECTANGLE_ = { "RECTANGLE", RECT_KEY_SIZE, 1, RECT_BLOCK_SIZE, wrap_rect_enc, wrap_rect_dec };
...
/* Named Stronger+Weaker throughout, matching benchmark5/6/8.c. */
...
int main(int argc, char **argv) {
    if (!rectangle_avx2_available()) {
        fprintf(stderr,
                "This CPU (or this build) does not support AVX2; benchmark2's "
                "RECTANGLE+HIGHT cascade now requires it (absorbed from "
                "benchmark7.c). Build/run on an AVX2-capable machine.\n");
        return 1;
    }
