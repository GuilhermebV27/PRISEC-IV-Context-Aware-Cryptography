/*
* benchmark3.c - PRISEC-IV Phase 3 Benchmark (AES-256+ChaCha20+AES-128
* triple cascade, hardware-accelerated only)
*
* [Formerly benchmark8.c / Phase 8. Stripped down to just the plain
* triple cascade running with native hardware acceleration (AES-NI +
* ChaCha20 SIMD both on, nothing disabled) - no ECC handshake variant, no
* software-only or mixed-hardware toggle passes. Those live in
* benchmark4.c's own AES-256+ChaCha20+AES-128 entries (BOTH_OFF,
* AES_ONLY_OFF, CHACHA_ONLY_OFF), so this file's single row is the
* natural "everything accelerated" baseline for that comparison.]
*
* Entry: AES-256+ChaCha20+AES-128
*   Encryption order: L1(AES-256) -> L2(ChaCha20) -> L3(AES-128).
*   Decryption order: L3 -> L2 -> L1.
*   Keys are fresh random bytes, regenerated every outer repeat
*   (benchmark2/4-style; no ECC handshake).
*
* Build:
    ...
    for (int c = 0; c < N_CASCADES; c++) {
        for (int s = 0; s < N_SIZES; s++) {
            ...
            printf("[parent] [%s | %s] finished -> enc_ms=%.4f dec_ms=%.4f mem_enc_peak_kb=%.4f (ovh=%.4f) mem_dec_peak_kb=%.4f (ovh=%.4f)\n\n",
                   CASCADES[c].name, SIZES[s].label, res.enc_ms, res.dec_ms,
